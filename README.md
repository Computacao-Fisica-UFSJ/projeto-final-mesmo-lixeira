...
O código é compatível com Arduino, além do ESP utilizado no projeto. Em teoria, é compatível com qualquer placa que utiliza drivers CH340 ou CP210x, embora apenas tenhamos testado o código com Arduino Uno e ESP8266 NodeMCU.
...
Todas as bibliotecas incluídas (`ESP8266WiFi.h`, `ESP8266HTTPClient.h` e `WiFiClient.h`) são built-in do pacote. Além disso, são apenas necessárias para a comunicação via WiFi (em que não obtivemos sucesso no trabalho).
...
python3 app.py
```
O requirements.txt foi gerado com o `pip freeze`, então talvez seja melhor usar `pip install flask` no lugar.

### Documentação do código de Arduino

#### Replicação visual do experimento

No código existe uma função responsável pela calibragem da luz quando o botão é pressionado (ou nos 3 primeiros do boot). É importante que o soro **NÃO** esteja gotejando enquanto a calibração está sendo executada (explicarei o porquê mais à frente).
...
É importante que o computador rodando o servidor esteja na mesma rede que o ESP. Ressalto isso, pois os computadores do laboratório não tem placa WiFi.
...
```
Os comentários no código já explicam bem como esta função funciona. Ela basicamente "amplifica" o sinal analógico, lendo `OVERSAMPLE_SIZE` valores do LDR, em vez de somente 1.
...
Na finalização, o *threshold* (`calibrated_threshold`) recebe o valor do ruído máximo (multiplicado por um número arbitrário, para adicionar uma margem de erro). Quanto maior for a margem de erro, menos falsos negativos, quanto menor, menos falsos positivos.
